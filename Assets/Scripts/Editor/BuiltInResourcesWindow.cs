...
public class BuiltInResourcesWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (_maxY > position.height && Event.current != null && Event.current.type == EventType.ScrollWheel)
        {
            float speed = 10;
            float movement = Event.current.delta.y * speed;
            _scrollPos = Mathf.Clamp(_scrollPos + movement, 0, _maxY);
            Repaint();
        }
    }
